<script>
    $(document).ready(function() {
        let isFetched = false;
        const defaultIDCardImgURL = @json($data->id_card_img_url ?? '');
        ...
        if(defaultIDCardImgURL){
            $('id-card-img').val("hello world");
            setPreviewImage(defaultIDCardImgURL);
        }
        ...
        function setPreviewImage(imageURL){
            $('#id-card-preview img').remove();
            $('#id-card-preview div').remove();
            const img = $('<img id="preview" class="object-cover z-10 relative">'); 
